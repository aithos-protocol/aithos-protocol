...
### Added
- Initial placeholder module reserving the import path
  `github.com/aithos-protocol/aithos` on proxy.golang.org.
- `ProtocolVersion = "0.1.0"` — the Aithos protocol version this module targets.
- `PackageVersion = "0.0.1"` — this module's own version.
- `CeremonialDID` — DID of the ceremonial founding identity that signed
  the protocol's first public artifacts.
- `ErrNotImplemented` and two stub functions (`VerifyBundle`, `ResolveDID`)
  that return it and point users to the TypeScript reference CLI.
- `BirthFS()` — an `io/fs.FS` rooted at the embedded birth artifacts:
...
- This release is a **name reservation** and a **timestamped birth record**.
  Nothing in this module is functional beyond exposing constants and reading the
  embedded artifacts.
