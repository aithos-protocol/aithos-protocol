...
function transactionalStorage() {
  ...
  // and the capability is DECLARED (0.13 formalizes the P1 note — without the
  // marker, an undefined identity on a non-delegated write refuses honestly).
  base.loadIdentity = async () => undefined;
  base.selfSigningWrites = true;
  const batches = [];
  let height = 3;
...
/** memoryStorage + extra public sections (incl. authored voice) + a circle zone. */
function incarnationStorage({ voiceBody } = {}) {
  const base = memoryStorage();
  const pub = new Map([
    ["sec_intro", { id: "sec_intro", title: "Hello", body: "I am Alice, I build Aithos.", gamma_ref: "g_i", tags: ["intro"] }],
    ["sec_bio", { id: "sec_bio", title: "Bio", body: "I build Aithos.", gamma_ref: "g_b", tags: ["pinned"] }],
    ["sec_work", { id: "sec_work", title: "Work", body: "Consulting + protocol.", gamma_ref: "g_w", tags: [] }],
  ]);
  if (voiceBody !== undefined) {
    pub.set("sec_voice", { id: "sec_voice", title: "Voice", body: voiceBody, gamma_ref: "g_v", tags: ["voice"] });
  }
  const circle = new Map([
    ["sec_secret", { id: "sec_secret", title: "Clients", body: "Secret client Zorglub pays well.", gamma_ref: "g_s", tags: ["pinned"] }],
  ]);
  const zones = { public: pub, circle };
  base.readSectionIndex = async (_h, zone) => {
    const m = zones[zone];
    if (!m) throw new Error(`no key for ${zone}`);
    return [...m.values()].map((s) => ({
      section_id: s.id, title: s.title, title_hidden: false, gamma_ref: s.gamma_ref, tags: s.tags,
    }));
  };
  base.readSections = async (_h, ids, opts = {}) => {
    const zone = opts.zone ?? "public";
    const m = zones[zone] ?? new Map();
    return ids.map((id) => {
      const s = m.get(id);
      return s
        ? { zone, section_id: id, accessible: true, section: s }
...
const AUTHORED_GUIDANCE = JSON.stringify({
  "aithos-guidance": "0.1.0",
  voice: { person: "first", languages: ["fr"], tone: ["direct"], formality: "casual", verbosity: "short" },
  rendering: { pinned_sections: ["sec_work"], transition_style: "natural", topic_hints: ["protocol"] },
  disclosure: { ai_disclosure: "always" },
  refusal_template: "Alice n'en parle pas dans son ethos.",
});

test("T9 — voice resource: authored public section wins; default §12.3.4 otherwise", async () => {
  // Authored: the body of the section tagged `voice`, verbatim.
  let { client } = await connect({ storage: incarnationStorage({ voiceBody: AUTHORED_GUIDANCE }) });
  let res = await client.readResource({ uri: "aithos://ethos/alice/voice" });
  assert.equal(res.contents[0].mimeType, "application/json");
  const authored = JSON.parse(res.contents[0].text);
  assert.equal(authored.voice.languages[0], "fr");
  assert.equal(authored.refusal_template, "Alice n'en parle pas dans son ethos.");

  // Default: §12.3.4 with {handle} substituted.
  ({ client } = await connect({ storage: incarnationStorage() }));
  res = await client.readResource({ uri: "aithos://ethos/alice/voice" });
  const dflt = JSON.parse(res.contents[0].text);
  assert.equal(dflt["aithos-guidance"], "0.1.0");
  assert.equal(dflt.voice.person, "first");
  assert.match(dflt.disclosure.disclosure_text, /narrating from alice's Aithos ethos/);
  assert.match(dflt.refusal_template, /outside what alice has recorded/);

  // The voice URI is listed, and zone URIs do not double-list it.
  const listed = (await client.listResources()).resources.map((r) => r.uri);
  assert.equal(listed.filter((u) => u === "aithos://ethos/alice/voice").length, 1);
});

test("T9b — prose voice section is served verbatim as text", async () => {
  const { client } = await connect({ storage: incarnationStorage({ voiceBody: "Écris court. Tutoie." }) });
  const res = await client.readResource({ uri: "aithos://ethos/alice/voice" });
  assert.equal(res.contents[0].mimeType, "text/markdown");
  assert.equal(res.contents[0].text, "Écris court. Tutoie.");
});

test("T17 — ethos_introduce serves ONLY public, anonymously; out-of-ethos → refusal template", async () => {
  // Owner session: full circle/self authority — introduce still never leaves public.
  const storage = incarnationStorage({ voiceBody: AUTHORED_GUIDANCE });
  const { client } = await connect({ storage });

  // Focus matching a CIRCLE-only term: zero matches (structural public-only).
  const out = text(await client.callTool({
    name: "ethos_introduce",
    arguments: { audience: "a recruiter", focus: "Zorglub" },
  }));
  assert.deepEqual(out.zones_served, ["public"]);
  assert.equal(out.matches.length, 0, "circle content never matches");
  assert.equal(out.refusal_template, "Alice n'en parle pas dans son ethos.");
  // The echoed `focus` is the caller's own input; the SERVED content
  // (headline/pinned/matches/guidance) must never carry circle material.
  const served = JSON.stringify([out.headline, out.pinned, out.matches, out.presentation_guidance]);
  assert.ok(!served.includes("Zorglub") && !served.includes("sec_secret"), "no circle leak in served content");

  // The introduction itself: headline (tag intro), authored pinned order, topics.
  assert.equal(out.headline.id, "sec_intro");
  assert.deepEqual(out.pinned.map((s) => s.id), ["sec_work"], "authored pinned_sections win");
  assert.deepEqual(out.topics, ["protocol"]);
  assert.equal(out.guidance_source, "authored");

  // In-ethos focus DOES match public content.
  const hit = text(await client.callTool({
    name: "ethos_introduce",
    arguments: { focus: "protocol" },
  }));
  assert.ok(hit.matches.length >= 1);
  assert.ok(hit.matches.every((m) => ["sec_intro", "sec_bio", "sec_work", "sec_voice"].includes(m.id)));
});

test("T7/narration — a self-scoped mandate neither lists nor serves ethos_introduce", async () => {
  const doc = liveMandate({
    actor_sphere: "self",
    scopes: ["ethos.read.self", "ethos.write.self"],
  });
  const { client } = await connect({
    storage: incarnationStorage(),
    mandate: { scopes: doc.scopes, document: doc },
  });
  const names = (await client.listTools()).tools.map((t) => t.name);
  assert.ok(!names.includes("ethos_introduce"), "narration hidden without public read");
  const forced = await client.callTool({ name: "ethos_introduce", arguments: {} });
  assert.equal(forced.isError, true, "forced narration call refuses (spec §4)");
});

test("V11 — agent_briefing composes mandate + voice + context pack in one call", async () => {
  // Owner session.
  const storage = incarnationStorage({ voiceBody: AUTHORED_GUIDANCE });
  let { client } = await connect({ storage });
  let b = text(await client.callTool({
    name: "agent_briefing",
    arguments: { task: "introduce the protocol work" },
  }));
  assert.equal(b.subject.handle, "alice");
  assert.equal(b.mandate.session, "owner");
  assert.equal(b.voice_profile.source, "authored");
  assert.equal(b.voice_profile.guidance.voice.languages[0], "fr");
  assert.equal(b.context_pack.task, "introduce the protocol work");
  assert.ok(b.context_pack.sections.length >= 1);
  assert.deepEqual(
    b.mandate.tools,
    (await client.listTools()).tools.map((t) => t.name).sort(),
    "briefing's announced tools == tools/list (T15 spirit)",
  );

  // Delegate session: pack is scope-bounded like context_pack itself.
  const doc = liveMandate({ scopes: ["ethos.read.public"] });
  ({ client } = await connect({
    storage: incarnationStorage({ voiceBody: AUTHORED_GUIDANCE }),
    mandate: { scopes: doc.scopes, document: doc },
  }));
  b = text(await client.callTool({
    name: "agent_briefing",
    arguments: { task: "protocol" },
  }));
  assert.equal(b.mandate.session, "delegate");
  assert.deepEqual(b.context_pack.zones_considered, ["public"]);
  assert.ok(b.context_pack.sections.every((s) => s.zone === "public"));
});

test("0.13 — a self-signing storage WITHOUT the marker refuses an identity-less write", async () => {
  const base = memoryStorage();
  base.loadIdentity = async () => undefined; // P1 hack, marker forgotten
  const { client } = await connect({ storage: base, autoCommit: true });
  const res = await client.callTool({
    name: "ethos_add_section",
    arguments: { zone: "public", title: "X", body: "y" },
  });
