...
| `ethos.write.public` | Append revisions to the `public` zone (delegated authoring; §4.5.4, §2.5.4). |
| `ethos.write.circle` | Append revisions to the `circle` zone. |
| `ethos.write.self` | Append revisions to the `self` zone. RECOMMENDED to carry `require_counter_sign`. |
...
A **write mandate** authorizes an agent to append revisions to a zone of the subject's ethos *without* holding the subject's sphere key. This is how a subject equips a secondary device, an editing tool, or an AI agent with the ability to amend their ethos while keeping the sphere key offline on the primary device.
...
- `grantee.pubkey` MUST be set. It is the multibase Ed25519 public key of the **delegate key** generated by the subject for this specific agent/device. Every revision signed under this mandate MUST be signed by that exact key.
- `constraints` MAY include additional keys specific to writing:
  - `sections: ["sec_a1b2c3", "sec_9f8e7d"]` — restrict the delegate to appending only to these section IDs. Omitted ⇒ any section in the authorized zone.
  - `max_revisions_per_day: N` — rate-limit on the number of revisions the delegate may commit per UTC day.
  - `max_body_bytes: N` — cap on the byte length of any single revision's `body` field.
  - `require_counter_sign: ["ethos.write.<zone>"]` — every revision authored by the delegate MUST carry a subject counter-signature to be considered valid. Turns the delegate into a drafting assistant rather than an autonomous author. RECOMMENDED for `self`.

When a delegate signs a revision, the revision object includes `authorized_by: "<mandate_id>"` and `signature.key` is the delegate's multibase Ed25519 public key — not a sphere DID URL. Verification is the combined procedure of §2.5.4.2 (revision chain) and §4.7 (mandate).
...
- Revisions committed with `R.at < revoked_at` remain valid and attributable.
- Revisions with `R.at ≥ revoked_at` are unauthorized regardless of the mandate's TTL.

If the subject believes a delegate key was compromised and wishes to **repudiate** past revisions authored by it, the subject issues public redaction revisions (§2.5.4.3) referencing those revisions by hash. The redaction is signed by the sphere key directly, leaves the original revisions in the chain, and makes the repudiation public and dated.
