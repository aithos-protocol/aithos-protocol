...
The schema is unified across `public`, `circle`, and `self`. Two zone-level discriminators are fixed by zone identity: the boolean `encrypted` flag (gates the per-section `cipher`), and the boolean `index_encrypted` flag (gates whether each section's title/tags live in clear on the descriptor or sealed in a per-section `title_cipher`).
...
| `index_encrypted` | boolean | `true` for `self`, absent/`false` for `public` and `circle`. Fixed per zone identity. When `true`, each `sections[].title`/`tags` MUST be absent (they live in that section's `title_cipher`); when absent/`false`, `sections[].title` is REQUIRED. This is the circle-clear / self-private compromise (§3.7′). |
...
| `sections[].title_cipher` | object | Encrypted section title — `alg`, `nonce`, `wraps`, `ct`. REQUIRED on each section when `index_encrypted: true`; MUST be absent otherwise. `ct` is the XChaCha20-Poly1305 ciphertext of `jcs({ title, tags? })`, sealed to **that section's** recipients (the same set as the section body). AAD = `"aithos-title-v1\0" ‖ subject_did ‖ "\0" ‖ section_id`. |
...
**Encrypted index (self) — per section.** The `self` zone hides each section's title from the host: the structural fields (`section_id`, `file`, `sha256_of_plaintext`, `cipher`, `gamma_ref`) stay in clear so a keyless verifier can still check integrity, file presence, and orphans, but the title/tags are sealed in that section's own `title_cipher`. The "index" is therefore the *union* of the per-section `title_cipher`s rather than one zone-wide blob. A reader recovers a section's title iff it is one of that section's recipients — so the subject (recipient of every section) sees all self titles, a section-scoped delegate sees only the titles of the sections it can read, and the host sees none. Browsing titles is still cheap (decrypt the small per-section title ciphers, not the section bodies).

This per-section design is what lets a section-scoped delegate **add/edit/delete its own** self sections (§ companion section-level-mandates draft): a new section simply carries its own `title_cipher` sealed to subject + the adding delegate — there is no zone-wide index blob to rebuild, so the delegate never needs the other titles. `circle` keeps a clear index by design (the host / circle members can browse circle titles). The section **count** and per-section **sizes** of `self` remain visible — only titles/tags are hidden; hiding the count is a further step (opaque filenames) deferred to a later revision (§3.13′).
...
- **`self`** encrypts each section's title. The **title** and **tags** of a self section are sealed in that section's `title_cipher`, readable by exactly that section's recipients — so the subject sees all self titles and a section-scoped delegate sees only its own. The host still sees, in clear, the self section **count**, **per-section sizes**, **recipient set**, and **gamma_ref** (the structural fields needed for keyless integrity checks), but **never the titles**.
...
- **Encrypted self index — DONE for titles (§3.3.2′ / §3.7′).** v0.3 seals each `self` section's title/tags into its own `title_cipher`, readable by that section's recipients (the circle-clear / self-private compromise; this per-section design lets a section-scoped delegate add/manage its own sections). What remains open is hiding the self section **count** and **ids**: today `self/<section_id>.enc` filenames and the clear `sections[]` length still reveal how many self sections exist and their (opaque) ids. Fully hiding the count needs opaque, content-addressed filenames and an encrypted `sections[]` blob, at the cost of keyless structural verification of `self`. Deferred — revisit if the count leak matters for a real threat model.
