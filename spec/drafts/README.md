...
|---|---|---|---|
| [`bundle-v0.3-per-section-encryption.md`](./bundle-v0.3-per-section-encryption.md) | §3 | **Promoted** | Split each zone into per-section blobs (one ciphertext file per section in `circle` / `self`, one plaintext markdown file per section in `public`). Editing one section costs O(section size) instead of O(zone size). Symmetric across all three zones. **Normative as of protocol-core 0.8.0 — v0.3 is the default on-disk format; v0.2 remains readable. Kept here as the per-section reference.** |
| [`bundle-v0.3-section-level-mandates.md`](./bundle-v0.3-section-level-mandates.md) | §3, §4 | **Promoted** | Per-section `title_cipher` + mandate `section_scope` so a section-scoped delegate can add/read/edit/delete exactly its own sections (e.g. an email capture-agent appending to `self`) without seeing the rest of the index. **Normative as of protocol-core 0.8.0.** |
